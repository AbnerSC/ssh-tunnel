...
class VertxSocks5Impl {
	...
	private void onConnectRequest(Buffer buffer) {
		...
		if (addrtype == 1) {// 0x01 IP V4地址
			...
			remoteAddr = new String(bytes, 5, addrLen);
		}
	...
	private void onAuth(Buffer buffer) {
		...
		String userName = new String(bytes, ++index, len);
		index += len;
		// 密码
		len = bytes[index];
		String password = new String(bytes, ++index, len);
		Logf.log("收到账号:%s", userName);
		// 验证账号密码
		if (this.userName.equals(userName) && this.password.equals(password)) {
			Logf.log("%s验证通过", userName);
			usedSocket.write(Buffer.buffer(new byte[] { 0x01, 0x00 }), (res) -> {
	...
	private void onAuthPre(Buffer buffer) {
		...
		if (buffer.length() > 2) {
			...
			// 设置账号密码认证
			usedSocket.write(Buffer.buffer(new byte[] { 5, 0x02 }), (res) -> {
	...
	private void onNotAuth(Buffer buffer) {
		...
		if (buffer.length() > 2) {
			...
			// 设置不需要认证
			usedSocket.write(Buffer.buffer(new byte[] { 5, 0 }), (res) -> {
				readDataHandler = this::onConnectRequest;
			});
		}
	}
	...
	private void writeAndClose(byte[] arr) {
		usedSocket.write(Buffer.buffer(arr), (res) -> usedSocket.close());
	}
